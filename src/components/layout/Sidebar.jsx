...
import { 
    ...
    Wallet, FileText, Settings, Shield 
} from 'lucide-react';
...
export default function Sidebar({
    ...
    // Clases dinámicas dependiendo de si la ficha del paciente está abierta (colapsado) o no
    ...
    return (
        <aside className={`fixed inset-y-0 left-0 z-50 ${sidebarWidth} transform ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 transition-all duration-300 ease-in-out bg-white border-r border-pastel-pink flex flex-col shadow-sm`}>
            ...
            <div className={`p-6 border-b border-pastel-pink/50 flex flex-col items-center gap-4 relative bg-warm-white/50 ${isWorkspaceActive ? 'px-2' : ''}`}>
                <button onClick={() => setMobileMenuOpen(false)} className="md:hidden absolute top-4 right-4 p-2 text-gray-400 hover:bg-pastel-pink/50 rounded-full transition-colors">
                    <X size={20}/>
                </button>
            ...
            <div className={`px-4 py-5 ${isWorkspaceActive ? 'px-2' : ''}`}>
                <div className={`flex items-center gap-3 p-3 rounded-2xl bg-warm-white border border-pastel-pink/50 shadow-sm hover:border-sage-green/50 transition-colors ${centerIcons}`}>
                    ...
                    <div className={`overflow-hidden flex-1 ${hideOnCollapse}`}>
                        ...
                        {/* Adaptamos los roles a la clínica psicológica */}
